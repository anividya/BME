...
function saveState() {
    ...
    localStorage.setItem('assetViewTableState', JSON.stringify(state));
}

function loadState() {
    const savedState = localStorage.getItem('assetViewTableState');
    if (savedState) {
...
function renderTable(data) {
    const tableBody = document.getElementById('assetViewTableBody');
    tableBody.innerHTML = '';
    data.forEach(row => {
        ...
        tr.innerHTML = `
            ...
            <td>${row.Status}</td>
            <td>
                <a href="#${modalText}" class="btn btn-warning btn-sm" data-toggle="modal" onclick="woid('${row.Work_ID}')">
