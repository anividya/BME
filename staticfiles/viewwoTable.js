...
const modalText = (UserGroup === 'BMEADMIN' || UserGroup === 'BMESTAFF') ? 'workModalBMEview' : 'workviewonly';
const functionText = (UserGroup === 'BMEADMIN' || UserGroup === 'BMESTAFF') ? 'woid' : 'woid2';
...
function saveState() {
    ...
    localStorage.setItem('viewwotableState', JSON.stringify(state));
}

function loadState() {
    const savedState = localStorage.getItem('viewwotableState');
    if (savedState) {
...
function renderTable(data) {
    const tableBody = document.getElementById('viewwotableBody');
    tableBody.innerHTML = '';
    data.forEach(row => {
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td><input type="checkbox" class="rowCheckbox" data-workid="${row.Work_ID}"> ${row.Work_ID}</td>
            ...
            <td>${row.Status}</td>
            <td>
                <a href="#${modalText}" class="btn btn-warning btn-sm" data-toggle="modal" onclick="${functionText}('${row.Work_ID}')">
