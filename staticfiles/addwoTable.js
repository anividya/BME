...
function saveState() {
    ...
    localStorage.setItem('addwotableState', JSON.stringify(state));
}

// Load the saved state from localStorage
function loadState() {
    const savedState = localStorage.getItem('addwotableState');
    if (savedState) {
...
function renderTable(data) {
    const tableBody = document.getElementById('addwotableBody');
    tableBody.innerHTML = '';
    data.forEach(row => {
    ...
    <td>
        ...
        <i class="fas fa-info"></i>Create WO
        </a>
    </td>
`;
...
function wodata_ajax(assetid, equipment, make, model, serialNumber, department) {
